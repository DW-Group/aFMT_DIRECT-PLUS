...
###############################################################################
# Calculate the increase and decrease of metabolites
###############################################################################
...
# Gain/upregulated model
metabolite_gain <- results_by_diet_treatment3[,c("SubjectID", "Diet", "Age_Quartile", "Gender", "WC_change_Quartile","Weight_change_Quartile","Richness_Quartile","aFMT", "Gain_Percent")]
metabolite_gain_model <- lm(Gain_Percent ~ Diet + Age_Quartile + Gender + WC_change_Quartile + Weight_change_Quartile + Richness_Quartile, metabolite_gain, na.action = na.exclude) 
metabolite_gain$predicted_value <- predict(metabolite_gain_model)
# Get the least-squares means for 'Diet'
lsmeans_metabolite_gain <- emmeans(metabolite_gain_model, ~ Diet)
lsmeans_metabolite_gain_df <- as.data.frame(lsmeans_metabolite_gain)[, c("Diet", "emmean")]
# Merge the LS‐Means Back into Your Data:
metabolite_gain <- metabolite_gain %>% left_join(lsmeans_metabolite_gain_df, by = "Diet")
# Calculate the Re-centered (Adjusted) Raw Values:
names(metabolite_gain)[9] <- "Value"
metabolite_gain <- metabolite_gain %>% mutate(adjusted_raw = Value - (predicted_value - emmean))


# Loss/dowregulated model
metabolite_loss <- results_by_diet_treatment3[,c("SubjectID", "Diet", "Age_Quartile", "Gender", "WC_change_Quartile","Weight_change_Quartile","Richness_Quartile","aFMT", "Loss_Percent")]
metabolite_loss_model <- lm(Loss_Percent ~ Diet + Age_Quartile + Gender + WC_change_Quartile + Weight_change_Quartile + Richness_Quartile, metabolite_loss, na.action = na.exclude) 
metabolite_loss$predicted_value <- predict(metabolite_loss_model)
lsmeans_metabolite_loss <- emmeans(metabolite_loss_model, ~ Diet)
lsmeans_metabolite_loss_df <- as.data.frame(lsmeans_metabolite_loss)[, c("Diet", "emmean")]
# Merge the LS‐Means Back into Your Data:
metabolite_loss <- metabolite_loss %>% left_join(lsmeans_metabolite_loss_df, by = "Diet")
# Calculate the Re-centered (Adjusted) Raw Values:
names(metabolite_loss)[9] <- "Value"
metabolite_loss <- metabolite_loss %>% mutate(adjusted_raw = Value - (predicted_value - emmean))


metabolite_gain$Metric <- "Increase" 
metabolite_loss$Metric <- "Decrease" 
metabolite_gainloss <- rbind(metabolite_gain, metabolite_loss)
...
annotation_df <- data.frame(
  ...
  predicted_value = c(24, 24),  # Adjust this y-value if needed
  Metric = c("Decrease", "Increase"),
  label = c("p = 2.02e-05", "p = 0.01")
)

metabolite_gainloss$Diet <- factor(metabolite_gainloss$Diet, levels = c("1", "2", "3"))
# Stratified by Diet and Treatment
ggplot(metabolite_gainloss, aes(x = factor(Diet), y = predicted_value, fill = factor(Diet))) +
    ...
    labs(
        title = "Metabolite change induced by diets, pre-aFMT",
        x = "",
        y = "% of metabolites"
    ) +
    facet_wrap(~ Metric, scales = "fixed",
        labeller = as_labeller(c( "Increase" = "Increase\n(0-6)",
                                  "Decrease" = "Decrease\n(0-6)"))
    ) +  # Separate plots by Strain Change
    geom_text(
    data = annotation_df,
...
library(openxlsx)
wb <- createWorkbook()
addWorksheet(wb, "Fig 3b Increased Metabolites")
writeData(wb, "Fig 3b Increased Metabolites", metabolite_gain)

addWorksheet(wb, "Fig 3b Decreased Metabolites")
writeData(wb, "Fig 3b Decreased Metabolites", metabolite_loss)

saveWorkbook(wb, "Source Data Fig 3b.xlsx", overwrite = TRUE)
