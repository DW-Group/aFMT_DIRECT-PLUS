###########################################################################
# Identify metabolites with higher Increase-Persistence in aFMT
###########################################################################
  ...
  for (metabolite in metabolite_cols) {
    ...
    temp_data <- temp_data %>%
      mutate(
        ...
        Persistently_Gain = ifelse(
          (Quartile_A == "Q1" & Quartile_B %in% c("Q3","Q4") & Quartile_D %in% c("Q3","Q4")) |
    ...
    temp_clean <- temp_data %>%
      select(SubjectID, Diet, Treatment, Persistently_Gain) %>%
      tidyr::drop_na() %>%
      distinct()
    ...
    counts_overall <- temp_clean %>%
      group_by(Treatment) %>%
      summarise(
        gain    = sum(Persistently_Gain == 1L, na.rm = TRUE),
        no_gain = sum(Persistently_Gain == 0L, na.rm = TRUE),
        .groups = "drop"
      )
    ...
    if (all(c("aFMT","Placebo") %in% counts_overall$Treatment)) {
      a <- counts_overall$gain[counts_overall$Treatment == "aFMT"]
      b <- counts_overall$no_gain[counts_overall$Treatment == "aFMT"]
      c_ <- counts_overall$gain[counts_overall$Treatment == "Placebo"]
      d <- counts_overall$no_gain[counts_overall$Treatment == "Placebo"]

      mat <- matrix(c(a, b, c_, d), nrow = 2, byrow = TRUE,
                    dimnames = list(Treatment = c("aFMT","Placebo"),
                                    Outcome = c("Gain","NoGain")))
    ...
    if (!is.null(metadata)) {
      ...
      td_use <- temp_clean %>% distinct(SubjectID, .keep_all = TRUE) %>% select(SubjectID,Persistently_Gain)

      temp_data2 <- left_join(md_use, td_use, by = "SubjectID")

      if (all(c("Persistently_Gain","Treatment","Diet") %in% names(temp_data2))) {
        temp_data2 <- temp_data2 %>%
          mutate(Treatment = factor(Treatment, levels = c("Placebo","aFMT")))

        vars <- c("Persistently_Gain","Treatment","Diet","Age_Quartile","Gender",
        ...
        ok_events <- sum(temp_cc$Persistently_Gain == 1L, na.rm = TRUE) >= 2

        if (ok_arms && ok_events) {
          model <- tryCatch(
            glm(Persistently_Gain ~ Treatment + Diet + Age_Quartile + Gender +
                  WC_change_Quartile + Weight_change_Quartile + Richness_Quartile,
    ...
    summary_tables <- list(
      overall_all = temp_clean %>%
        summarise(n = n(), Persistently_Gain = sum(Persistently_Gain, na.rm = TRUE), .groups = "drop") %>%
    ...
    for (nm in names(summary_tables)) {
      summary_tables[[nm]] <- summary_tables[[nm]] %>%
        mutate(
          ...
          Persistently_Gain_p = ifelse(n > 0, round(Persistently_Gain * 100 / n, 2), NA_real_),
          # attach ORs (same for every row of this metabolite)
          #odds_ratio   = OR_unadjusted,
          #ci_low = OR_unadj_ci_low,
          #ci_high= OR_unadj_ci_high,
          #p_val      = OR_unadj_p,
        ...
        select(Metabolite, Diet, Treatment, n, Persistently_Gain, Persistently_Gain_p,
...
#fecal_chem_annotation <- read.table("../Fecal_chem_annotation.tsv", header=T, sep="\t", check.names = FALSE, fill = TRUE)
library(readr)
...
#output_file <- "~/mydata/Project_aFMT/metaphlan_v411/metaphlan_all_species/6.ER_per_species_results/per_metabolite_increase_persistence_results.tsv"
...
writeData(wb, "Increased-Persist Metabolites", per_metabolite_transition_results)

saveWorkbook(wb, "Supplementary Table 6.xlsx", overwrite = TRUE)
...
fecal_chem_annotation_merged_filt <- fecal_chem_annotation_merged %>% filter(CHEMICAL_NAME %in% fecal_chem_annotation_merged$CHEMICAL_NAME) %>% 
  ...
  filter(
    if_all(1, ~ {
      p_aFMT <- Persistently_Gain_p[Diet == 3 & Treatment == "aFMT"]
      p_placebo <- Persistently_Gain_p[Diet == 3 & Treatment == "Placebo"]
      length(p_aFMT) == 0 || length(p_placebo) == 0 || p_aFMT > p_placebo
    })
  ) %>%
  ungroup()

fecal_chem_annotation_merged2 <- fecal_chem_annotation_merged_filt  %>% filter(Diet=="Overall" & Treatment=="Overall")  %>% filter(Persistently_Gain > 2 & odds_ratio > 1 & odds_ratio <=15 & ci_high < 250  & SUPER_PATHWAY != "NA") %>% distinct()
...
fecal_chem_annotation_ranked <- fecal_chem_annotation_merged2 %>% arrange(desc(odds_ratio))
...
#[1] 16.444444  1.555556

#To remove metabolites where Persistently_Gain_p in Diet = 2 & Treatment = Placebo is greater than that in Diet = 2 & Treatment = aFMT, you can do the following:
...
diet2_wide <- diet2 %>%
  select(Metabolite, Treatment, Persistently_Gain_p) %>%
  pivot_wider(names_from = Treatment, values_from = Persistently_Gain_p)
...
filtered_data <- filtered_data %>%
  mutate(CHEMICAL_NAME = str_replace_all(CHEMICAL_NAME, c("alpha" = "α", "beta" = "β", "gamma" = "γ")) )
filtered_data[which(filtered_data$CHEMICAL_NAME=="ala-ile-ala"),"CHEMICAL_NAME"] = "Alanine-isoleucine-alanine"
...
#################################################
metabolites_rm <- c("100005383","100004056","100009261","100008914","100015759","100000285")

filtered_data2 <- filtered_data2 %>% filter(!Metabolite %in% metabolites_rm)

ggplot(filtered_data2, aes(x = CHEMICAL_NAME, 
...
wb <- createWorkbook()
addWorksheet(wb, "Increased-Persist Metabolites")
writeData(wb, "Increased-Persist Metabolites", filtered_data2)
saveWorkbook(wb, "Source Data Fig 3e.xlsx", overwrite = TRUE)
...
fecal_chem_annotation_merged_bubble <- fecal_chem_annotation_merged_bubble %>% filter(Metabolite %in% filtered_data2$Metabolite)
...
# bubble_plot
p <- ggplot(fecal_chem_annotation_merged_bubble, aes(x = Treatment_diet, y = CHEMICAL_NAME)) +
    geom_point(aes(size = Persistently_Gain_p, color = Treatment_diet, alpha=Persistently_Gain_p/38) ) +
...
p
# Output portrait 7*5
#("~/mydata/Project_aFMT/metaphlan_v411/metaphlan_all_species/Figure3e. Increase−Persistence metabolites filtered bubble plot.pdf", plot = p, width = 7.5, height = 6)
...
p <- ggplot(fecal_chem_annotation_merged_bubble, aes(x = CHEMICAL_NAME, fill = SUPER_PATHWAY)) +
    geom_bar(alpha=0.8) +
    coord_flip() +
...
p
# Save to PDF
#ggsave("~/mydata/Project_aFMT/metaphlan_v411/metaphlan_all_species/Figure3e. Increase−Persistence metabolites filtered super pathway.pdf", plot = p, width = 6.2, height = 6)
# Output portrait 6.2*6
...
setwd('~/Downloads')
increase_persistence_df <- read_tsv("increase_persistence_functional_categories.tsv", show_col_types = FALSE)
head(increase_persistence_df)
# A tibble: 6 × 2
#  metabolite                             functional_category                       
#  <fct>                                  <chr>                                     
#1 5α-pregnan-3β,20β-diol monosulfate (1) Gut-brain and tryptophan-related signaling
#2 serotonin                              Gut-brain and tryptophan-related signaling
#3 5-dodecenoylcarnitine (C12:1)          Fatty-acid oxidation and energy metabolism
#4 10-nonadecenoate (19:1n9)              Fatty-acid oxidation and energy metabolism
#5 5-aminoimidazole-4-carboxamide         Amino-acid, peptide, and nucleotide metab…
#6 lactobacillic acid                     Structural and membrane lipid metabolism  

# Capitalize only metabolites that start with a letter, while leaving those starting with numbers/symbols unchanged.
increase_persistence_df <- increase_persistence_df %>%
  mutate(
    metabolite = if_else(
...
increase_persistence_df2 <- increase_persistence_df
increase_persistence_df2$metabolite <- as.character(increase_persistence_df2$metabolite)
increase_persistence_df2[which(increase_persistence_df2$metabolite=="1-(1-enyl-palmitoyl)-2-palmitoyl-GPC (P-16:0/16:0)*"),"metabolite"] = "1-(1-enyl-palm...GPC (P-16:0/16:0)*"
increase_persistence_df2[which(increase_persistence_df2$metabolite=="1-linoleoyl-2-linolenoyl-galactosylglycerol (18:2/18:3)*"),"metabolite"] = "1-linoleoyl-2-lino...glycerol (18:2/18:3)*"
#increase_persistence_df2$metabolite <- factor(increase_persistence_df2$metabolite)
increase_persistence_df2 <- increase_persistence_df2 %>%
  mutate(metabolite = factor(metabolite, levels = rev(metabolite)))
  
p <- ggplot(increase_persistence_df2, aes(x = metabolite, fill = functional_category)) +
    geom_bar(alpha=1) +
    coord_flip() +
...
p
# Save to PDF
#ggsave("~/mydata/Project_aFMT/metaphlan_v411/metaphlan_all_species/Figure3e. Increase−Persistence metabolites filtered functional category.pdf", plot = p, width = 8.8, height = 6)
